...
3. `GeminiProvider`, `MistralProvider` e `GroqProvider` cuidam do protocolo
   HTTP, parsing, retry e rotação entre chaves do mesmo provedor.
4. `RateLimitScheduler` serializa chamadas e controla o intervalo entre os
   inícios reais das requisições de cada provedor.

Uma factory explícita cria um scheduler e um provider por integração, todos com
o mesmo relógio e dependências injetadas. Não existe registro global de
schedulers. O serviço
...
- Somente os provedores explicitamente cadastrados no catálogo local: Gemini,
  Mistral e Groq.
...
## Revisão de 2026-09-04 — terceiro provedor

- **Resultado:** manter.
- **Motivo:** a inclusão da Groq reutilizou as fronteiras existentes sem exigir
  event bus, container de DI ou registro dinâmico de plugins. O gateway ganhou
  roteamento explícito por agente e modelo; cada provider continua responsável
  somente pelo próprio protocolo, parsing, retry e rotação de chaves.
- **Escopo mantido:** catálogo estático e revisável de provedores e modelos. Uma
  quarta integração exige nova revisão desta ADR antes da implementação.
