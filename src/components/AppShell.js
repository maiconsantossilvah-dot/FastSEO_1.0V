...
function openSidebar() {
  ...
  window.setTimeout(() => $('appSidebar')?.querySelector('button')?.focus(), 60);
}
  ...
  const observer = new MutationObserver(mutations => {
    mutations.forEach(mutation => {
      ...
      mutation.removedNodes.forEach(node => {
        ...
        if (lastModalTrigger instanceof HTMLElement) window.setTimeout(() => lastModalTrigger.focus(), 0);
      });
    });
    if (mutations.some(mutation => mutation.addedNodes.length)) queueIconRefresh();
  });
...
function renderThemeIcon(mode) {
  ...
  button.innerHTML = `<i data-lucide="${isLight ? 'moon' : 'sun'}" aria-hidden="true"></i>`;
