...
export const SubcatModal = {
  ...
  async _exportXlsx() {
    ...
    const dados = SubcatModule.getAll().map(rule => {
      const customizada = !SUBCAT_RULES_DEFAULT.some(defaultRule =>
        defaultRule.nome === rule.nome &&
        defaultRule.formula === rule.formula
      );
      ...
      return {
        ...
        Exemplo: rule.ex || '',
        Customizada: customizada ? 'Sim' : 'Nao',
      };
    });
    ...
    window.XLSX.utils.book_append_sheet(arquivo, planilha, 'Titulos');

    window.XLSX.writeFile(arquivo, 'subcategorias_titulos.xlsx');

    Utils.showToast('Planilha exportada.');
  },
  async _resetAll() {
