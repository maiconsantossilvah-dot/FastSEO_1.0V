...
export async function callGemini(system, userMsg, maxTokens, attempt = 1, signal = null) {
  ...
  // 503 / 529 ou "overloaded" no corpo → sinaliza para o roteador tentar a outra API
  if (res.status === 503 || res.status === 529) {
    const e   = await res.json().catch(() => ({}));
    const msg = e?.error?.message || `HTTP ${res.status}`;
    PipelineUI.log(`⚠ Gemini indisponível (${res.status}): ${msg} — tentando Mistral...`, 'w');
    throw Object.assign(new Error('cota_esgotada'), { cotaEsgotada: true });
  }
  ...
  if (!res.ok) {
    const e   = await res.json().catch(() => ({}));
    const msg = e?.error?.message || '';
    if (/overloaded|service.?unavailable|capacity|too many/i.test(msg)) {
      PipelineUI.log(`⚠ Gemini sobrecarregado: ${msg} — tentando Mistral...`, 'w');
      throw Object.assign(new Error('cota_esgotada'), { cotaEsgotada: true });
    }
...
export async function callMistral(system, userMsg, maxTokens, signal = null, attempt = 0) {
  ...
  if (res.status === 503 || res.status === 529) {
    ...
    PipelineUI.log(`⚠ Mistral indisponível (${res.status}): ${msg} — tentando Gemini...`, 'w');
    throw Object.assign(new Error('cota_esgotada'), { cotaEsgotada: true });
  }
  ...
  if (!res.ok) {
    ...
    const msg = e?.message || '';
    if (/overloaded|service.?unavailable|capacity|too many/i.test(msg)) {
      PipelineUI.log(`⚠ Mistral sobrecarregado: ${msg} — tentando Gemini...`, 'w');
      throw Object.assign(new Error('cota_esgotada'), { cotaEsgotada: true });
    }
